...
def save_dataframe(df: pd.DataFrame, out_file):
  ...
  if out_file.endswith(".tsv"):
    ...
    df.to_csv(out_file, sep="\t", index=False, quotechar='"', escapechar='"',
              line_terminator="\n")
...
def replace_format(filename, format_override):
  ...
  """
  format_override = format_override if format_override.startswith(
      ".") else "." + format_override
  p = Path(filename)
...
def add_filename_suffix(filename, suffix, format_override=None):
  ...
  file_format = file_format if file_format.startswith(
      ".") else "." + file_format
  return "{0}_{1}{2}".format(Path.joinpath(p.parent, p.stem), suffix,
                             file_format)


def add_column_prefix(df: pd.DataFrame, prefix: str, columns_to_rename=None,
    columns_to_keep=None) -> pd.DataFrame:
  """
  ...
  df.columns = [col if col in columns_to_keep else prefix + col for col in
                columns_to_rename]
  return df
...
def remove_empty_strings(df: pd.DataFrame, columns) -> pd.DataFrame:
  ...
  for col in columns:
    if col in df.columns:
      df[col] = [None if isinstance(v, str) and len(v) == 0 else v for v in
                 df[col]]
...
def remove_empty_lists_values(df: pd.DataFrame, columns) -> pd.DataFrame:
  ...
  for col in columns:
    if col in df.columns:
      df[col] = [None if isinstance(v, list) and len(v) == 0 else v for v in
                 df[col]]
...
def groupby_join_unique_values(df: pd.DataFrame, columns,
    as_lists: bool = False,
  ...
  if as_lists:
    ...
    return df.groupby(columns).agg(
        lambda col: join_unique(col, str_separator)).reset_index()
...
def combine_dfs_fill_missing_values(target: pd.DataFrame,
...
def get_unique_list(input_list, ignore_case: bool = True):
  seen = set()
  if ignore_case:
    return [x for x in input_list if
            notnull(x) and x.lower() not in seen and not seen.add(x.lower())]
  else:
    return [x for x in input_list if
            notnull(x) and x not in seen and not seen.add(x)]
...
def save_chunks(df: pd.DataFrame, base_filename, n_chunks=4, suffix="chunk") -> \
    list[pd.DataFrame]:
