...
from pandas_utils import remove_empty_lists_values, isnull
...
def npatlas_url(structure: str, search_type: str = "inchikey", similarity: str = "exact"):
    url = None
    if similarity == "exact":
    ...
    if url is None:
        raise ValueError("url is None this means npatlas search had wrong search or similarity type")
    return url
...
def search_np_atlas_by_inchikey_smiles(row, search_similar: bool = False):
    ...
    result = None
    if "inchikey" in row:
        result = query_npatlas(row["inchikey"], search_type="inchikey", similarity=similarity)
    if result is None and "smiles" in row:
        result = query_npatlas(row["smiles"], search_type="smiles", similarity=similarity)
    if result is None and "canonical_smiles" in row:
        result = query_npatlas(row["canonical_smiles"], search_type="smiles", similarity=similarity)

    return result


@memory.cache
def query_npatlas(structure: str, search_type: str = "inchikey", similarity: str = "exact"):
    """
    ...
    """
    if isnull(structure) or (isinstance(structure, str) and len(structure) == 0):
        return None
...
def search_np_atlas(df: pd.DataFrame,
    ...
    filtered = create_expired_entries_dataframe(df, MetaColumns.date_npatlas, refresh_expired_entries_after)

    filtered["results"] = filtered.apply(lambda row: search_np_atlas_by_inchikey_smiles(row), axis=1)
    # refresh date
    filtered.loc[filtered["results"].notnull(), MetaColumns.date_npatlas] = iso_datetime_now()
